...
checkpointer = callbacks.ModelCheckpoint(filepath="logs/dls/cnn/checkpoint-{epoch:02d}.hdf5", save_best_only=True, monitor='loss')
csv_logger = CSVLogger('logs/dls/cnn/training_set_lstmanalysis.csv',separator=',', append=False)
model.fit(X_train, y_train, batch_size=32, epochs=1000, shuffle=True,callbacks=[checkpointer,csv_logger])
model.save("logs/dls/cnn/completemodel.hdf5")
